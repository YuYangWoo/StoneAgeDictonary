...
public class RecommendDeckActivity extends AppCompatActivity {
    ...
    private void InitializePetData() {
        ...
        DeckList.add(new Recommend("추천 펫","샌디쟈드, 라비루, 푸테라, 만모, 모가로스, 오투투 ,킹북이, 문울프, 카키, 얀기로, 토끼, 핑코, 골드로비, 크라톤","현재 가장 많이 사용하는 펫들을 모아놓았습니다."));
        DeckList.add(new Recommend( "OP","샌디쟈드 + 푸테라 + 만모 + 핑코(3성여우) + 라비루"," 여전히 OP입니다. 라비루의 순발을 최대로 끌어올려서 라비루의 궁 후 핑코의 궁으로 기력을 제거하고 뚜까 패는 덱입니다."));
        DeckList.add(new Recommend( "OP","토끼 + 만모 + 모가로스 + 얀기로 + 오투투","토끼 액티브로 -상태이상 버프(여우카운터) + 액티브딜로 첫 턴에 백여우를 죽이는 등 첫 턴에 한명은 삭제됩니다. 취향에 따라 킹북이를 넣곤 합니다."));
        ...
        DeckList.add(new Recommend( "OP","토끼 + 만모 + 모가로스 + 얀기로 + 라비루","토끼 액티브로 -상태이상 버프(여우카운터) + 안정성과 딜도 빵빵함"));
