...
public class RecommendDeckActivity extends AppCompatActivity {
    ...
    private void InitializePetData() {
        ...
        DeckList.add(new Recommend("추천 펫","샌디쟈드, 라비루, 푸테라, 만모, 모가로스, 오투투, 문울프, 카키, 듀포드, 토끼, 핑코, 골드로비, 티라고스","현재 가장 많이 사용하는 펫들을 모아놓았습니다.\n 듀포드를 선발에 넣는게 정말 안정적이네요."));
        DeckList.add(new Recommend("기계침입 덱","쟈그, 샌디쟈드, 라비루, 모가로스, 크랩, 기로, 얀기로","기계침입에서 많이 쓰는 펫을 모았습니다."));
        DeckList.add(new Recommend( "OP","샌디쟈드(메카키) + 티라고스 + 듀포드 + 핑코(3성여우) + 라비루"," 말할필요 없이 국민덱"));
        DeckList.add(new Recommend( "OP","샌디쟈드(메카키) + 골드로비 + 듀포드 + 핑코(3성여우) + 라비루"," 티라고스 패시브로 안전하게 2턴 여우 매혹으로 이어지는 완벽한 덱 !"));
        DeckList.add(new Recommend( "OP","티라고스 + 핑코 + 만모 + 라비루 + 샌디쟈드","여우로 매혹시키고 패는 환상적인 덱"));
        DeckList.add(new Recommend( "OP","라비루 + 백여우 + 골드로비 + 샌디쟈드 + 만모","골드로비의 사기적인 보호막으로 백여우의 안전을 책임! 유지력이나 pvp정말 좋습니다."));
        DeckList.add(new Recommend( "OP","티라고스 + 만모 + 모가로스 + 샌디쟈드 + 라비루","토끼 액티브로 -상태이상 버프(여우카운터) + 안정성과 딜도 빵빵함"));
